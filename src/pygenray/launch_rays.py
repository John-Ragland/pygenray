...
def shoot_rays(
    ...
    cin, cpin, rin, zin, depths, depth_ranges, bottom_angles = _unpack_envi(
        environment, flatearth=flatearth
    )

    # check that coordinates are monotonically increasing
    if not (np.all(np.diff(rin) >= 0)):
        raise Exception(
            "Sound speed range coordinates must be monotonically increasing."
        )
    if not (np.all(np.diff(zin) >= 0)):
        raise Exception(
            "Sound speed depth coordinates must be monotonically increasing."
        )
        ...
        try:
            ...
            for k, single_ray in enumerate(rays_ls):
                if single_ray is None:
                    ...
                    rays_list[rays_list_idx].launch_angle = -launch_angles[
                        k
            ...
            time.sleep(
                0.1
            )  # Ensure all processes have finished before cleaning up shared memory
...
def _unpack_envi(environment, flatearth=True):
    ...
    if flatearth:
        ...
        if not hasattr(environment, "sound_speed_fe"):
            raise Exception(
                "Flat earth transformation has not been applied. Set `flat_earth_transform=True` when creating the OceanEnvironment2D object."
            )
...
def _interpolate_ray(sols: list, range_save: np.array):
    ...
    full_ray = np.concatenate(
        (full_ray, np.expand_dims(sols[-1].y[:, -1], axis=1)), axis=1
    )

    # Append range values to full_ray
    full_ray_state = np.concatenate(
        (np.expand_dims(range_save, axis=0), full_ray), axis=0
    )
