...
class RayFan:
    ...
    def __init__(self, Rays: list):
        ...
        if len(Rays) < 2:
            raise ValueError(
                f"RayFan requires at least 2 surviving rays to build a ray fan, "
    ...
    def compute_rayids(self):
        ...
        """
        ray_ids = np.sum(np.diff(np.sign(self.ps)) != 0, axis=1) * (
            np.sign(self.thetas)
        )
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            if key < 0 or key >= len(self.thetas):
                raise IndexError(
                    f"Index {key} is out of bounds for RayFan with {len(self.thetas)} rays"
                )
    ...
    def __init__(
        ...
        for ridx in range(len(receiver_depths)):
            ...
            for eray_idx in range(eray_fan.rs.shape[0]):
                y_last = np.stack(
                    (
                        eray_fan.ts[eray_idx, -1],
                        eray_fan.zs[eray_idx, -1],
                        eray_fan.ps[eray_idx, -1],
                    )
                )
                theta, c = pr.ray_angle(
                    eray_fan.rs[eray_idx, -1],
                    y_last,
                    environment.sound_speed.values,
                    environment.sound_speed.range.values,
                    environment.sound_speed.depth.values,
                )
                received_angles_single.append(theta)
                ray_id_single = np.sum(
                    np.diff(np.sign(eray_fan.ps[eray_idx, :])) != 0
                ) * (np.sign(eray_fan.thetas[eray_idx]))
    ...
    def save_mat(self, filename):
        ...
        for ridx, rdepth in enumerate(self.receiver_depths):
            data[f"receiver_depth_{ridx}"] = {
                ...
                "source_depth": self.source_depths[ridx]
                if hasattr(self, "source_depths")
                else np.nan,
