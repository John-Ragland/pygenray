...
class TestBackwardsShooting:
    ...
    def test_endpoints_and_p_constant(self):
        ...
        c0 = 1500.0
        env = _const_c_env(c0=c0)
        ray = shoot_ray(200.0, 30e3, -10.0, 0.0, 60,
                        env, rtol=1e-9, flatearth=False, debug=False)
        assert ray is not None
        assert ray.r[0] == 30e3
        assert ray.r[-1] == 0.0
    ...
    def test_matches_manually_mirrored_environment(self):
        ...
        ssp_m = xr.DataArray(c_2d[::-1, :], dims=['range', 'depth'],
                             coords={'range': r, 'depth': z})
        bathy_m = xr.DataArray(bathy_vals[::-1], dims=['range'], coords={'range': r})
        env_m = OceanEnvironment2D(sound_speed=ssp_m, bathymetry=bathy_m,
                                   flat_earth_transform=False)

        source_range, receiver_range = 60e3, 10e3
        angle = -15.0

        ray_bwd = shoot_ray(200.0, source_range, angle, receiver_range, 80,
                            env, rtol=1e-9, flatearth=False, debug=False)
        ray_fwd_mirrored = shoot_ray(
            200.0, 100e3 - source_range, angle, 100e3 - receiver_range, 80,
            env_m, rtol=1e-9, flatearth=False, debug=False,
        )
        ...
        assert (ray_bwd.n_bottom, ray_bwd.n_surface) == \
            (ray_fwd_mirrored.n_bottom, ray_fwd_mirrored.n_surface)
    ...
    def test_shoot_rays_backwards_matches_shoot_ray(self):
        ...
        rf = shoot_rays(200.0, 40e3, angles, 5e3, 60, env,
                        rtol=1e-9, flatearth=False, debug=False)
        assert len(rf) == len(angles)
        assert np.allclose(rf.rs[:, 0], 40e3)
        assert np.allclose(rf.rs[:, -1], 5e3)

        idx = np.argmin(np.abs(rf.thetas - 7.0))
        single = shoot_ray(200.0, 40e3, rf.thetas[idx], 5e3, 60,
                           env, rtol=1e-9, flatearth=False, debug=False)
