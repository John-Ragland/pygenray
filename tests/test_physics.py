...
def _const_c_env(
    c0=1500.0, z_max=5000.0, r_max=100e3, bathy_depth=4500.0, nz=200, nr=20
):
    ...
    return OceanEnvironment2D(
        sound_speed=ssp, bathymetry=bathy, flat_earth_transform=False
    )


def _linear_gradient_env(
    c0=1500.0, g=0.05, z_max=5000.0, r_max=100e3, bathy_depth=4500.0, nz=500, nr=50
):
...
class TestConstantSSPStraightLine:
    ...
    def test_travel_time_analytical(self):
        ...
        ray = shoot_ray(
            z0, 0.0, user_angle, R, 50, env, rtol=1e-9, flatearth=False, debug=False
        )
...
def _regenerate_fixture():
    ...
    rf = shoot_rays(
        1300.0, 0.0, angles, 50e3, 50, env, n_processes=1, debug=False, flatearth=False
    )
...
class TestMunkRegression:
    ...
    def test_regression(self, request):
        regenerate = request.config.getoption("--regenerate-physics", default=False)
        ...
        np.testing.assert_allclose(
            rf.ts, ref["ts"], atol=1e-6, err_msg="Travel times changed"
        )
        np.testing.assert_allclose(rf.zs, ref["zs"], atol=0.1, err_msg="Depths changed")
        np.testing.assert_allclose(
            rf.ps, ref["ps"], atol=0.1, err_msg="Ray parameters changed"
        )
